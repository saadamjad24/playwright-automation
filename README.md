...
### What it does:
This automation performs a full e-commerce checkout workflow for three different user types using a data-driven JSON configuration file.

### Process:
1. Reads configuration from `week5_config.json` containing credentials, product lists, and checkout details for `standard_user`, `problem_user`, and `performance_glitch_user`.
2. For each user, the script:
   - Logs into the site
   - Adds specified products to the cart by matching product names
   - Navigates to the cart to verify items (logging names and prices)
   - Fills out checkout information
   - Extracts Item Total, Tax, and Grand Total from the final overview page
   - Takes a full-page screenshot of the order confirmation page
   - Logs out
3. Finally, it generates a summary JSON file for each user recording the items ordered, totals, status, and timestamp.

### Setup & Run:
```bash
npx playwright test Automation/week5.js --project=chromium --headed
```

### Output files generated:
- `outputs/week5/summary_<username>.json` — summary JSON per user
- `outputs/week5/order_confirmation_<username>.png` — confirmation screenshot (or error state screenshot on failure)